...
import framing  # shared crop math — preview (Studio) == this render by construction
...
def probe_dims(video: Path) -> tuple[int, int]:
    """Source pixel width/height (for reframe crop math). Falls back 1920x1080."""
    try:
        out = subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0",
...
def extract_segment(
    ...
    if geom_filter:
        # Reframe (e.g. 16:9 → 9:16): crop+scale computed by framing.py so the
        # Studio preview the user framed against and this render are identical.
        scale = geom_filter
    elif draft:
        scale = "scale=1280:-2"
    else:
...
def extract_all_segments(
    ...
    target = framing.parse_target(edl.get("target"))
    dims_cache: dict[str, tuple[int, int]] = {}

    seg_paths: list[Path] = []
    print(f"extracting {len(ranges)} segment(s) → {clips_dir.name}/")
    if target:
        print(f"  (reframing to {target[0]}x{target[1]} per-range)")
    if is_auto:
    ...
    for i, r in enumerate(ranges):
        ...
        geom_filter = None
        if target:
            if src_name not in dims_cache:
                dims_cache[src_name] = probe_dims(src_path)
            sw, sh = dims_cache[src_name]
            geom_filter = framing.ffmpeg_reframe_filter(
                sw, sh, target[0], target[1], r.get("frame"))

        note = r.get("beat") or r.get("note") or ""
        print(f"  [{i:02d}] {src_name}  {start:7.2f}-{end:7.2f}  ({duration:5.2f}s)  {note}")
        if target:
            print(f"        reframe: {geom_filter}")
        if is_auto:
            print(f"        grade: {seg_filter or '(none)'}")
        extract_segment(src_path, start, duration, seg_filter, out_path,
                        preview=preview, draft=draft, geom_filter=geom_filter)
...
def build_final_composite(
    ...
    if has_subs:
        print("  skipping subtitle burn-in due to missing libass, but master.srt was generated")
